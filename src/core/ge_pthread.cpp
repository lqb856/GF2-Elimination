...
void *ge_pthread_core(void *args) {
  ...
  int row_per_thread = std::ceil(static_cast<double>(ER_MATRIX.num_rows_) /
                                 static_cast<double>(num_thread));
  int range_start = thread_id * row_per_thread;
  int range_end = (thread_id + 1) * row_per_thread;
  if (range_start >= ER_MATRIX.num_rows_)
    return nullptr;
  if (range_end >= ER_MATRIX.num_rows_)
    range_end = ER_MATRIX.num_rows_;

  // mutex.lock();
  // std::cout << "thread " << thread_id << " range " << range_start << " - " <<
  // range_end << std::endl; mutex.unlock();
